...
fn build_mocap(bvh: &bvh::Bvh) -> Mocap {
    ...
    for _ in 0..bvh.motion.num_frames {
        pull_frame_data(&mut ret.root, &bvh.motion.frame_data, &mut frame_data_index);
    }
...
fn pull_frame_data(joint: &mut Joint, frame_data: &[f64], frame_data_index: &mut usize) {
    for channel in joint.channels.iter_mut() {
        channel.values.push(frame_data[*frame_data_index] as _);
        *frame_data_index += 1;
    }

    if let JointChildren::Joints(ref mut joints) = joint.children {
        for joint in joints.iter_mut() {
            pull_frame_data(joint, frame_data, frame_data_index);
        }
    }
}
